...
2）收集资讯。Twitter 优先通过 `twitter-cli` 收集 list，这样可以直接保留更完整的媒体信息（尤其是图片 URL / 尺寸），失败时再回退到 [twitterapi.io](http://twitterapi.io/)。Substack 则通过登录态列出已订阅 publications，再抓取每个 publication 的最新文章。两边都做来源级增量采集，避免重复吃旧内容。Twitter 采集完成后还要多做一层 source 归一化：抽取 tweet 正文和最近 1-3 条 replies 里的外链，轻量读取候选页面；如果 tweet 只是把官方页面当宣发渠道做总结和介绍，那么后续流程一律把这个官方链接当主 source，原 tweet 链接只保留作审计元数据。
